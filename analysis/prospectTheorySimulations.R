...
plotProspect <- function(accuracy,target) {
  # plotting the expected gain for the two responses.
  #arguments: accuracy: points offered for correct responses
  # target: points awarded or deducted when the target stimulus is presented
  ...
  expected_reward_target = probWeight(ptarget)*(valueFunction(accuracy+target))+
    probWeight(1-ptarget)*valueFunction(0);
  expected_reward_nontarget = probWeight(ptarget)*valueFunction(target)+
    probWeight(1-ptarget)*valueFunction(accuracy);
  ...
  ggsave(paste('figures/modelling/prospect_',accuracy,'_',target,'.png',sep=''),
         width=6,height=4)
}
