...
In Exp. 7, associating stimulus absence with point loss had 7 significant effect on participants' decision bias. The proportion of target-presence reports in absence-loss (`r E7.bias$noise_loss%>%mean()%>%printnum()`) was significantly higher than in neutral blocks (`r E7.bias$neutral%>%mean()%>%printnum()`; `r E7.bias$noise_loss_effect%>%t.test()%>%apa_print()%>%'$'(full_result)`). 
